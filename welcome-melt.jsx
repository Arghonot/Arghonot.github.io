/* welcome-melt.jsx — DOOM-style ordered-dither "melt" that dissolves the
...
function WelcomeMelt({
  ...
  React.useEffect(() => {
    ...
    const frame = (now) => {
      ...
      if (reveal >= 1) { canvas.style.opacity = '0'; return; }
  ...
  return (
    <div aria-hidden="true" style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: 'rgb(0,128,128)' }}>
      ...
      {showScroll && (
        <div className="wm-scrolldown">
          <span className="wm-scrolltext">
            {window.WordArt
              ? <window.WordArt variant="spectrum" lines={['Scroll down']} size={40} />
              : <span style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontWeight: 700, fontSize: 40, color: '#eafcff', textShadow: '1px 1px 0 rgba(0,0,0,.45)' }}>Scroll down</span>}
          </span>
